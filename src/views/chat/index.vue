...
function handleChangeWallpaper(keyword: string) {
	addChat(
		+uuid,
		{
			...
			requestOptions: { prompt: keyword, options: null },
			wallpaperKeyword: keyword,
			wallpaperUrl: ''
		},
	)
	scrollToBottom()
...
function updateChatItem(index: number, obj: any) {
...
async function onConversation() {
	...
	prompt.value = ''
	if (message.startsWith('change wallpaper:')) {
		const keyword = message.replace(/change wallpaper:/, '').trim();
		if (keyword) return handleChangeWallpaper(keyword)
	}
		...
		<main class="flex-1 overflow-hidden">
			<div id="scrollRef" ref="scrollRef" class="h-full overflow-hidden overflow-y-auto">
				<div id="image-wrapper" class="w-full max-w-screen-xl m-auto dark:bg-[#101014]"
					...
					<template v-else>
						<div>
							<Message v-for="(item, index) of dataSources" :key="index + (item.wallpaperUrl || '')"
								:date-time="item.dateTime" :text="item.text" :inversion="item.inversion" :error="item.error"
								:loading="item.loading" :wallpaperKeyword="item.wallpaperKeyword" :wallpaperUrl="item.wallpaperUrl"
								@updateChatItem="updateChatItem(index, $event)" @regenerate="onRegenerate(index)"
